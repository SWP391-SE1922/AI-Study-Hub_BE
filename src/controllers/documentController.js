...
const downloadDocumentVersion = asyncHandler(async (req, res) => {
  ...
  return res.download(result.downloadUrl, result.fileName, (err) => {
    if (err) {
      ...
      if (!res.headersSent) {
        res.status(500).json({ success: false, message: 'Không thể tải phiên bản file về' });
      }
    }
  });
});
...
/**
 * Xem trước một phiên bản cụ thể của tài liệu
 */
const previewDocumentVersion = asyncHandler(async (req, res) => {
  const currentUser = req.user || null;
  const version = await documentService.previewDocumentVersion(
    currentUser,
    req.params.id,
    req.params.versionId
  );
  ...
  return sendSuccess(
    res,
    'Lấy thông tin xem trước phiên bản tài liệu thành công',
    { version },
    null,
    200
  );
