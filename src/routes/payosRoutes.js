...
router.post('/create_payment_link', authMiddleware, async function (req, res) {
    ...
    try {
        ...
        if (orderCode) {
            await markTransactionFailed({ paymentMethod: PAYMENT_METHOD.PAYOS, orderCode })
                .catch((dbError) => console.error('PayOS transaction update error:', dbError));
        }
    ...
    try {
        ...
        if (webhookData.code === '00') {
            ...
            const result = await markTransactionSuccess({
                ...
                providerTransactionId: webhookData.reference,
                description: webhookData.description
            });

            if (result.count === 0) {
                console.warn(`PayOS IPN: Không tìm thấy giao dịch chờ xử lý ${webhookData.orderCode} hoặc giao dịch đã được ghi nhận`);
            }
