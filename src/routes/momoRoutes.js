...
router.post('/create_payment_url', authMiddleware, async function (req, res) {
    ...
    try {
        ...
        if (data.resultCode === 0) {
            ...
            await markTransactionFailed({ paymentMethod: PAYMENT_METHOD.MOMO, orderCode: orderId });
            res.status(500).json({ message: "Lỗi tạo thanh toán MoMo", data: data });
        }
    } catch (error) {
        if (orderId) {
            await markTransactionFailed({ paymentMethod: PAYMENT_METHOD.MOMO, orderCode: orderId })
                .catch((dbError) => console.error('MoMo transaction update error:', dbError));
        }
 ...
 */
router.get('/momo_return', async function (req, res) {
    const {
        partnerCode,
        orderId,
        requestId,
 ...
 */
router.post('/momo_ipn', async function (req, res) {
    const {
        partnerCode,
        orderId,
        requestId,
